...
class ERGraphGenerator:
    def __init__(self, min_n, max_n, cl, max_size_exec_percent, p_connection_low, p_connection_high, max_epochs):
        assert min_n <= max_n
        assert p_connection_low <= p_connection_high

        if cl:
            self.start_min_n = 20
            self.start_max_n = 30
        else:
            self.start_min_n = min_n
            self.start_max_n = max_n

        self.cl_percent_epochs = max_size_exec_percent

        self.min_n = min_n
        self.max_n = max_n
        self.cur_max = self.start_max_n
        self.cur_min = self.start_min_n
        self.p_connection_low = p_connection_low
        self.p_connection_high = p_connection_high
        self.cl = cl  # Curriculum Learning
        self.max_epochs = max_epochs

    # allow the trainer to modify the active range on the fly
    def set_bounds(self, min_n: int, max_n: int):
        self.min_n = int(min_n)
        self.max_n = int(max_n)

    def _pick_size(self, problem_size, cur_epoch):
        if problem_size is not None:
            return int(problem_size)

        if not self.cl or self.max_epochs <= 0 or cur_epoch is None:
            low, high = self.min_n, self.max_n
        else:
            progress = np.clip((cur_epoch + 1) / self.max_epochs, 0.0, 1.0)
            if progress <= self.cl_percent_epochs:
                t = progress / self.cl_percent_epochs
                cur_min = int(round(self.start_min_n + t * (self.min_n - self.start_min_n)))
                cur_max = int(round(self.start_max_n + t * (self.max_n - self.start_max_n)))
            else:
                cur_min, cur_max = self.min_n, self.max_n

            self.cur_min = max(cur_min, 1)
            self.cur_max = max(cur_max, cur_min)
            low, high = self.cur_min, self.cur_max

        # randint is [low, high) -> +1 to include high
        return int(np.random.randint(low, high + 1)) if low != high else int(low)

    def _sample_p(self, batch_size):
        if self.p_connection_low == self.p_connection_high:
            return [self.p_connection_low] * batch_size
        return np.random.uniform(self.p_connection_low, self.p_connection_high, batch_size)

    def generate_graphs(self, batch_size, problem_size=None, seed=None, cur_epoch=None):
        if seed is not None:
            np.random.seed(seed)

        n_nodes = self._pick_size(problem_size, cur_epoch)
        p_connection = self._sample_p(batch_size)

        batch = np.zeros((batch_size, n_nodes, n_nodes), dtype=np.int32)
        for b in range(batch_size):
            g = nx.erdos_renyi_graph(n_nodes, p_connection[b])
...
class RBGraphGenerator:
    def __init__(self, graph_sizes):
        # small (200-300) large (800-1200)
        self.graph_sizes = graph_sizes

        self.min_n_small = 200
        self.max_n_small = 300
        self.min_n_large = 800
        self.max_n_large = 1200

    def generate_graphs(self, batch_size, problem_size=None, seed=None, cur_epoch=None):
        if self.graph_sizes == 'mixed':
            # Select small or large randomly
            cur_graph_size = 'small' if np.random.random() < 0.5 else 'large'
        else:
            cur_graph_size = self.graph_sizes

        if cur_graph_size == 'small':
            min_n, max_n = self.min_n_small, self.max_n_small
        else:
            min_n, max_n = self.min_n_large, self.max_n_large
        ...
        while True:
            g, _ = get_random_instance(size=cur_graph_size)
...
def get_random_instance(size="small"):
    if size == "small":
        ...
        na = np.random.randint(40, 55)
        k = np.random.randint(20, 25)
    else:
        raise NotImplementedError

    p = np.random.uniform(0.3, 1.0)
    a = np.log(k) / np.log(na)
    re = - a / np.log(1 - p)

    i = generate_instance(na, k, re, p)
    G = nx.Graph()
...
coloring_language = Constraint_Language(domain_size=3, relations={'NEQ': [[0, 1], [0, 2], [1, 0], [1, 2], [2, 0], [2, 1]]})

is_language = Constraint_Language(domain_size=2, relations={'NAND': [[0, 0], [0, 1], [1, 0]]})
...
mc_weighted_language = Constraint_Language(domain_size=2, relations={'EQ': [[1, 1], [0, 0]], 'NEQ': [[1, 0], [0, 1]]})
...
class CSP_Instance:
    ...
    def graph_to_weighted_mc_instance(graph, name=None):
        """
        :param graph: A NetworkX graphs
        :param name: The relation name to assign to each edge
        :return: A CSP Instance representing the graph
        """
        adj = nx.linalg.adjacency_matrix(graph)
        n_variables = adj.shape[0]
